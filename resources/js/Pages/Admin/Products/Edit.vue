...
onMounted(() => {
    const keys = Object.keys(form.attributes);
    if (keys.length > 0) {
        attributeKeys.value = keys;
    } else {
        attributeKeys.value = [''];
    }
});
...
const removeAttributeField = (index) => {
    ...
    if (keyToRemove && form.attributes[keyToRemove] !== undefined) {
...
const updateAttributeKey = (oldKey, newKey, index) => {
    if (oldKey && oldKey !== newKey && form.attributes[oldKey] !== undefined) {
        ...
        form.attributes = newAttributes;
    } else if (newKey && form.attributes[newKey] === undefined) {
        form.attributes[newKey] = '';
    }
};
...
<template>
    <Head :title="`${product.name} - Düzenle - Admin Panel`" />
        ...
        <form @submit.prevent="submit" class="space-y-6">
            <div class="bg-white p-6 rounded-lg shadow">
                ...
                <div class="mt-6">
                    ...
                    <ImageUploader v-model="form.images" @update:modelValue="handleImagesUpdate" :multiple="true" :maxFiles="5" />
            ...
            <div class="bg-white p-6 rounded-lg shadow">
                ...
                <div v-if="attributeKeys.length === 0 || (attributeKeys.length === 1 && !attributeKeys[0])" class="text-gray-500 text-center py-4">
